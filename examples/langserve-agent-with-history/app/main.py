...
# =============================================================================
# The code below is for adapting the LangServe app to Pluto.
#   1. Remove the FastAPI app creation and route definitions from global scope.
#   2. Add the FastAPI app creation and route definitions to a function, and return the app.
#   3. Create a raw_handler function that returns a Mangum handler for the FastAPI app.
#   4. Create a Router instance and add a route that forwards all requests to the raw_handler.
...
def return_fastapi_app():
    ...
    # If you want to use LangServe's playground, add this route and access it at
    # `<url_from_pluto>/dev/playground`. Once PR https://github.com/langchain-ai/langserve/pull/580
    # gets merged, we might not need this route anymore.
    add_routes(
        app,
        agent_executor.with_types(input_type=Input, output_type=Output).with_config(
            {"run_name": "agent"}
...
def raw_handler(*args, **kwargs):
    # `api_gateway_base_path` is required, and the value should only be `/dev` now.
    handler = Mangum(return_fastapi_app(), api_gateway_base_path="/dev")
    return handler(*args, **kwargs)


router = Router("langserve")
