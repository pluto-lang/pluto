...
Pluto 是一种新型开源编程语言，旨在帮助开发者编写云应用程序，降低云服务的使用难度。开发者可以根据业务需求，在代码中以变量的形式定义所需资源，例如 KV 数据库、消息队列等。Pluto 通过分析代码获取应用依赖的基础设施资源，以及它们之间的依赖关系，并在指定的云平台上部署相应的资源实例和应用程序。
...
可以在[这篇文章](./docs/zh-CN/whats-different.md)中详细了解与其他产品的不同之处。
...
Pluto 目前还处于 PoC 阶段，欢迎感兴趣的人参与贡献，无论是对 Pluto 要解决的问题、提供的特性，还是代码实现有任何建议或者想法，都请随时[与我们联系](#)。想要参与代码开发可以参考 [DEVELOPMENT](#) 指南。

参与空间有：

1. 静态推导过程实现
   1. 资源类型检查
   2. local 变量转换 cloud 资源
2. SDK 研发
   1. API SDK 研发
   2. IaC SDK 研发
   3. 更多资源、更多平台，优先后者
